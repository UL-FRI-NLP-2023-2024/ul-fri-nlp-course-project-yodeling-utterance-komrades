...
class MultiTaskClassifier(nn.Module):
    ...
    def __init__(self, model_name: str, config: Dict[str, any]):
        ...
        # Construct the classifier body. Both the multi-label and multi-class classifiers share the same body,
        # only the output layer differs.
        self.classifier = nn.Sequential(*layers)   

        # Add the output layers for the multi-label and multi-class classifiers.
        self.fc1 = nn.Linear(self.hidden_layer_size, self.output_classes).to(self.device)
        self.fc2 = nn.Linear(self.hidden_layer_size, self.output_classes_sentiment).to(self.device)
    ...
    def forward(self, x: List[str]) -> torch.Tensor:
        ...
        # for the multi-class classifier (sentiment).
        logits = self.classifier(embeddings)
        logits_out = self.fc1(logits)
        sentiment = self.fc2(logits.to(self.device))

        return logits_out, sentiment.to(self.device)
    ...
    def _extract_config(self, config: Dict[str, any]):
        ...
        self.output_classes_sentiment = config['output_classes_sentiment']
